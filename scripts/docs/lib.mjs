...
export const PRODUCT_ID = "ableton-mcp";
export const RELEASE_REPOSITORY = "cavi-ai/ableton-mcp";
...
function packageVersion() {
  ...
  if (typeof version !== "string") throw new Error("package.json version is required");
  return version;
}
...
export function isEntrypoint(meta) {
  try {
    return Boolean(process.argv[1]) && realpathSync(process.argv[1]) === meta.filename;
  } catch {
    return false;
  }
}
