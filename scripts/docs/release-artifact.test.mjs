...
test("release artifact, checksum, and exact envelope are deterministic", async (context) => {
  const temporary = await mkdtemp(path.join(os.tmpdir(), "ableton-mcp-docs-release-"));
  ...
  assert.deepEqual(results[0].envelope, {
    ...
    artifact: {
      url: `https://github.com/cavi-ai/ableton-mcp/releases/download/${IDENTITY.tag}/ableton-mcp-docs-${IDENTITY.tag}.tar.gz`,
      sha256,
      format: "tar.gz",
    },
  });
  assert.equal(
    ...
    `${sha256}  ableton-mcp-docs-${IDENTITY.tag}.tar.gz\n`,
  );
  const names = tarNames(firstBytes);
  assert.ok(names.includes("cavi-release.json"));
  assert.ok(names.every((name) => name === "cavi-release.json" || name.startsWith(`docs/ableton-mcp/v${DOCUMENTED_VERSION}/`)));
});

test("release artifact rejects documentation built for another commit", async (context) => {
  const temporary = await mkdtemp(path.join(os.tmpdir(), "ableton-mcp-docs-provenance-"));
  context.after(() => rm(temporary, { recursive: true, force: true }));
  const { buildDocumentation } = await import("./build.mjs");
  const { createProductDocsReleaseArtifact } = await import("./release-artifact.mjs");
  const docsRoot = path.join(temporary, "docs");
