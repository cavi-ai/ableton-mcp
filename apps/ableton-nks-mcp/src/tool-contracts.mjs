...
export const toolContracts = {
  ...
  analyze_audio_file: { description: "Measure local source audio loudness, true peak, format, optional spectral peaks, spectrogram and monophonic pitch. Not Live processing or pitch correction. Requires ffmpeg and ffprobe.", inputSchema: object({ sourcePath: string("Absolute local audio file."), includeSpectrogram: boolean("Include up to 64 time-resolved spectral frames, resampled to 48 kHz."), includePitch: boolean("Estimate first-channel monophonic pitch over 0.256 seconds; requires that minimum window."), includeSpectrum: boolean("Include one 4096-sample spectral frame at the window start."), startSeconds: number("Window start.", { minimum: 0 }), durationSeconds: number("Window duration, defaults to 10 seconds.", { exclusiveMinimum: 0, maximum: 60 }) }, ["sourcePath"]) },
  analyze_audio_clip: { description: "Measure an exact Session or Arrangement audio clip's local source file over a bounded window. Source-only, not rendered clip or track audio.", inputSchema: object({ trackId: ids.trackId, clipId: ids.clipId, includeSpectrogram: boolean("Include up to 64 first-channel spectral frames at 48 kHz."), includePitch: boolean("Estimate first-channel monophonic pitch over 0.256 seconds; not pitch correction."), includeSpectrum: boolean("Include one first-channel 4096-sample spectral frame."), startSeconds: number("Source window start.", { minimum: 0 }), durationSeconds: number("Source window duration, defaults to 10 seconds.", { exclusiveMinimum: 0, maximum: 60 }) }, ["trackId", "clipId"]) },
  ...
  set_rack_chain_note_routing: { description: "Plan or reassign a Drum Rack chain to a MIDI pad note and optionally change its instrument output note. Occupied destinations layer chains rather than replace sounds.", inputSchema: guarded({ trackId: ids.trackId, deviceId: ids.deviceId, chainId: string("Exact rack chain ID."), inputNote: { type: "integer", minimum: 0, maximum: 127 }, outputNote: { type: "integer", minimum: 0, maximum: 127 } }, ["trackId", "deviceId", "chainId"]) },
  set_drum_pad_state: { description: "Plan or mute/solo an exact populated Drum Rack pad by MIDI note with a guarded rack snapshot. Does not assign sounds to pads.", inputSchema: guarded({ trackId: ids.trackId, deviceId: ids.deviceId, note: { type: "integer", minimum: 0, maximum: 127 }, mute: boolean("Mute pad."), solo: boolean("Solo pad.") }, ["trackId", "deviceId", "note"]) },
  rename_rack_chain: { description: "Plan or rename an exact rack chain with a guarded hierarchy snapshot. For return chains, name is a raw label; Live adds the return letter prefix. Use observed name for display, not verbatim restoration.", inputSchema: guarded({ trackId: ids.trackId, deviceId: ids.deviceId, chainId: string("Exact chain ID."), name: string("New layer name; raw unprefixed label for returns.") }, ["trackId", "deviceId", "chainId", "name"]) },
  set_rack_chain_mixer: { description: "Plan or apply exact rack-chain volume, pan, mute, and solo changes using native ranges and guarded hierarchy state.", inputSchema: guarded({ trackId: ids.trackId, deviceId: ids.deviceId, chainId: string("Exact chain ID from device hierarchy."), volume: number("Native chain volume value, not decibels."), pan: number("Native chain pan value."), mute: boolean("Mute chain."), solo: boolean("Solo chain.") }, ["trackId", "deviceId", "chainId"]) },
