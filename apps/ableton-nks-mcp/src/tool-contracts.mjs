...
export const toolContracts = {
  ...
  duplicate_session_object: { description: "Plan or duplicate an exact track, Session scene, or clip.", inputSchema: guarded({ targetType: { type: "string", enum: ["track", "scene", "clip"] }, targetId: string("Stable source ID."), trackId: ids.trackId }, ["targetType", "targetId"]) },
  ...
  set_master_mixer: { description: "Plan or apply guarded master volume, pan, cue-volume, or crossfader changes.", inputSchema: guarded({ volume: number("Master volume."), pan: number("Master pan.", { minimum: -1, maximum: 1 }), cueVolume: number("Cue volume."), crossfader: number("Crossfader position.", { minimum: -1, maximum: 1 }) }) },
  ...
  transform_midi_notes: { description: "Plan or apply guarded quantize, legato, or duplicate transforms to exact MIDI note IDs.", inputSchema: guarded({ trackId: ids.trackId, clipId: ids.clipId, operation: { type: "string", enum: ["quantize", "legato", "duplicate"] }, noteIds: array({ type: "integer" }, "Stable note IDs."), gridBeats: number("Quantization grid in beats.", { exclusiveMinimum: 0 }), amount: number("Quantization amount.", { minimum: 0, maximum: 1 }), offsetBeats: number("Duplication offset in beats.") }, ["trackId", "clipId", "operation", "noteIds"]) },
