...
class DispatchTest(unittest.TestCase):
    ...
    def test_guarded_chord_inversion_rederives_pitch_and_preserves_complete_note_state(self):
        song = Song()
        clip = song.tracks[0].clip_slots[0].clip
        high, low, middle = MidiNote(7), MidiNote(8), MidiNote(9)
        high.pitch = 67
        ...
        by_id = {note["noteId"]: note for note in before["notes"]}
        changes = [
            {"noteId": 8, "previous": by_id[8], "pitch": 72},
            {"noteId": 9, "previous": by_id[9], "pitch": 64},
            {"noteId": 7, "previous": by_id[7], "pitch": 67},
        ]
        params = {**target, "expectedStateVersion": 3, "before": before, "clipTiming": timing,
                  "gridReference": {"tempoBpm": 120.0,
                                    "timeSignature": {"numerator": 4, "denominator": 4},
                                    "setFingerprint": dispatch_request(song, {"method": "get_live_state"}, 3)["setFingerprint"]},
                  "operation": "apply_midi_chord_inversion",
                  "chordInversion": {"noteIds": [7, 8, 9], "direction": "up",
                                     "steps": 1, "changes": changes},
                  "changes": changes, "newNotes": []}
        tampered = {**params, "changes": [{**changes[0], "pitch": 71}, *changes[1:]]}
        with self.assertRaisesRegex(ValueError, "signed plan"):
            dispatch_request(song, {"method": "transform_midi_notes", "params": tampered}, 3)
        result = dispatch_request(song, {"method": "transform_midi_notes", "params": params}, 3)
        observed = {note["noteId"]: note for note in result["notes"]}
        self.assertEqual([observed[note_id]["pitch"] for note_id in (8, 9, 7)], [72, 64, 67])
        self.assertEqual([observed[note_id]["probability"] for note_id in (8, 9, 7)], [0.75] * 3)
        self.assertEqual([observed[note_id]["velocityDeviation"] for note_id in (8, 9, 7)], [-3] * 3)
        self.assertEqual(song.undo_boundaries, ["begin", "end"])
    ...
    def test_guarded_chord_voice_leading_rederives_pitch_and_rejects_tampering(self):
        song = Song()
        clip = song.tracks[0].clip_slots[0].clip
        notes = [MidiNote(note_id) for note_id in range(7, 15)]
        for index, (note, pitch) in enumerate(zip(notes, (60, 64, 67, 71, 53, 57, 60, 64))):
            note.pitch = pitch
            note.start_time = 0.0 if index < 4 else 1.0
            note.velocity = 91
            note.velocity_deviation = -3
            note.release_velocity = 62
            note.probability = 0.75
        ...
        before = dispatch_request(song, {"method": "get_midi_clip_notes_extended", "params": target}, 3)
        timing = dispatch_request(song, {"method": "get_clip_timing", "params": target}, 3)
        expected = (60, 64, 67, 71, 65, 69, 72, 76)
        changes = [{"noteId": note["noteId"], "previous": note, "pitch": pitch}
                   for note, pitch in zip(before["notes"], expected)]
        note_ids = list(range(7, 15))
        params = {**target, "expectedStateVersion": 3, "before": before, "clipTiming": timing,
                  "gridReference": {"tempoBpm": 120.0, "timeSignature": {"numerator": 4, "denominator": 4},
                                    "setFingerprint": dispatch_request(song, {"method": "get_live_state"}, 3)["setFingerprint"]},
                  "operation": "apply_midi_chord_voice_leading",
                  "chordVoiceLeading": {"noteIds": note_ids, "mode": "all_voices", "minPitch": 48,
                                         "maxPitch": 84, "totalMovementSemitones": 20, "changes": changes},
                  "changes": changes, "newNotes": []}
        tampered = {**params, "changes": [*changes[:4], {**changes[4], "pitch": 53}, *changes[5:]]}
        with self.assertRaisesRegex(ValueError, "signed plan"):
            dispatch_request(song, {"method": "transform_midi_notes", "params": tampered}, 3)
        result = dispatch_request(song, {"method": "transform_midi_notes", "params": params}, 3)
        observed = {note["noteId"]: note for note in result["notes"]}
        self.assertEqual([observed[note_id]["pitch"] for note_id in note_ids], list(expected))
        self.assertEqual([observed[note_id]["probability"] for note_id in note_ids], [0.75] * 8)
        self.assertEqual(song.undo_boundaries, ["begin", "end"])
    ...
    def test_guarded_probability_pattern_applies_probability_and_preserves_complete_note_state(self):
        ...
        before = dispatch_request(song, {"method": "get_midi_clip_notes_extended", "params": target}, 3)
        timing = dispatch_request(song, {"method": "get_clip_timing", "params": target}, 3)
        changes = [{"noteId": 7, "previous": before["notes"][0], "probability": 0.75},
                   {"noteId": 8, "previous": before["notes"][1], "probability": 0.25}]
        params = {**target, "expectedStateVersion": 3, "before": before, "clipTiming": timing,
                  "gridReference": {"tempoBpm": 120.0,
                                    "timeSignature": {"numerator": 4, "denominator": 4},
                                    "setFingerprint": dispatch_request(song, {"method": "get_live_state"}, 3)["setFingerprint"]},
                  "operation": "apply_midi_probability_pattern",
                  "probabilityPattern": {"noteIds": [7, 8], "probabilities": [0.75, 0.25],
                                         "changes": changes},
                  "changes": changes, "newNotes": []}
        result = dispatch_request(song, {"method": "transform_midi_notes", "params": params}, 3)
        self.assertEqual([note["probability"] for note in result["notes"]], [0.75, 0.25])
        self.assertEqual([note["duration"] for note in result["notes"]], [1.0, 1.0])
        self.assertEqual(song.undo_boundaries, ["begin", "end"])
    ...
    def test_guarded_chord_arpeggiation_rederives_transform_and_rejects_tampering(self):
        song = Song()
        clip = song.tracks[0].clip_slots[0].clip
        notes = [MidiNote(note_id) for note_id in (7, 8, 9, 10)]
        for note, pitch in zip(notes, (60, 64, 67, 71)):
        ...
        before = dispatch_request(song, {"method": "get_midi_clip_notes_extended", "params": target}, 3)
        timing = dispatch_request(song, {"method": "get_clip_timing", "params": target}, 3)
        changes = [{"noteId": note.note_id, "previous": before["notes"][index],
                    "start": index * 0.25, "duration": 0.2}
                   for index, note in enumerate(notes)]
        new_notes = [
            {"sourceNoteId": note_id, "pitch": pitch, "start": start, "duration": 0.2,
             "velocity": 91, "velocityDeviation": -3, "releaseVelocity": 62,
             "probability": 0.75, "mute": False}
            for note_id, pitch, start in ((9, 67, 1.0), (8, 64, 1.25))
        ]
        arpeggiation = {"noteIds": [7, 8, 9, 10], "mode": "up_down",
                        "stepBeats": 0.25, "gate": 0.8, "seed": 7,
                        "changes": changes, "newNotes": new_notes}
        params = {**target, "expectedStateVersion": 3, "before": before, "clipTiming": timing,
                  "gridReference": {"tempoBpm": 120.0,
                                    "timeSignature": {"numerator": 4, "denominator": 4},
                                    "setFingerprint": dispatch_request(song, {"method": "get_live_state"}, 3)["setFingerprint"]},
                  "operation": "apply_midi_chord_arpeggiation", "chordArpeggiation": arpeggiation,
                  "changes": changes, "newNotes": new_notes}
        tampered = copy.deepcopy(params)
        tampered["changes"][1]["start"] = 0.3
        tampered["chordArpeggiation"]["changes"][1]["start"] = 0.3
        with self.assertRaisesRegex(ValueError, "arpeggiation changes"):
            dispatch_request(song, {"method": "transform_midi_notes", "params": tampered}, 3)
        result = dispatch_request(song, {"method": "transform_midi_notes", "params": params}, 3)
        self.assertEqual(result["addedNoteIds"], [100, 101])
        added = {note["noteId"]: note for note in result["notes"]}
        self.assertEqual([added[note_id]["pitch"] for note_id in (7, 8, 9, 10, 100, 101)],
                         [60, 64, 67, 71, 67, 64])
        self.assertEqual([added[note_id]["start"] for note_id in (7, 8, 9, 10, 100, 101)],
    ...
    def test_guarded_diatonic_transposition_binds_live_scale_and_rejects_tampering(self):
        song = Song()
        clip = song.tracks[0].clip_slots[0].clip
        notes = [MidiNote(note_id) for note_id in (7, 8, 9)]
        for note, pitch in zip(notes, (60, 64, 71)):
            note.pitch = pitch
            note.velocity = 91
            note.probability = 0.75
        clip.extended_notes = notes
        ...
        changes = [{"noteId": note_id, "previous": before["notes"][index],
                    "pitch": pitch, "degree": degree}
                   for index, (note_id, pitch, degree) in enumerate(((7, 62, 2), (8, 65, 4), (9, 72, 1)))]
        diatonic = {"noteIds": [7, 8, 9], "scaleSteps": 1,
        ...
        params = {**target, "expectedStateVersion": 3, "before": before,
                  ...
                  "gridReference": {"tempoBpm": 120.0,
                                    "timeSignature": {"numerator": 4, "denominator": 4},
                                    "setFingerprint": dispatch_request(song, {"method": "get_live_state"}, 3)["setFingerprint"]},
                  "operation": "apply_midi_diatonic_transposition",
                  "midiDiatonicTransposition": diatonic, "changes": changes, "newNotes": []}
        tampered = copy.deepcopy(params)
        tampered["changes"][2]["pitch"] = 74
        tampered["midiDiatonicTransposition"]["changes"][2]["pitch"] = 74
        with self.assertRaisesRegex(ValueError, "diatonic-transposition changes"):
            dispatch_request(song, {"method": "transform_midi_notes", "params": tampered}, 3)
        changed_scale = copy.deepcopy(params)
        changed_scale["musicalContext"]["key"]["rootNote"] = 2
        with self.assertRaisesRegex(ValueError, "musical context changed"):
            dispatch_request(song, {"method": "transform_midi_notes", "params": changed_scale}, 3)
        result = dispatch_request(song, {"method": "transform_midi_notes", "params": params}, 3)
        observed = {note["noteId"]: note for note in result["notes"]}
        self.assertEqual([observed[note_id]["pitch"] for note_id in (7, 8, 9)], [62, 65, 72])
        self.assertEqual([observed[note_id]["probability"] for note_id in (7, 8, 9)], [0.75, 0.75, 0.75])
        self.assertEqual(song.undo_boundaries, ["begin", "end"])

    def test_guarded_diatonic_harmony_rederives_scale_voices_and_rejects_tampering(self):
        song = Song()
        clip = song.tracks[0].clip_slots[0].clip
        notes = [MidiNote(note_id) for note_id in (7, 8)]
        ...
        musical = dispatch_request(song, {"method": "get_song_musical_context", "params": {}}, 3)
        new_notes = []
        for source, pitches in zip(before["notes"], ((64, 67), (67, 71))):
            for degree_offset, pitch in zip((2, 4), pitches):
                new_notes.append({"sourceNoteId": source["noteId"], "degreeOffset": degree_offset,
                                  "pitch": pitch, "start": source["start"], "duration": source["duration"],
                                  "velocity": 91, "velocityDeviation": -3, "releaseVelocity": 62,
                                  "probability": 0.75, "mute": False})
        harmony = {"noteIds": [7, 8], "degreeOffsets": [2, 4],
                   "scale": {"rootNote": 0, "scaleName": "Major",
                             "scaleIntervals": [0, 2, 4, 5, 7, 9, 11]},
                   "newNotes": new_notes, "preservedNotes": before["notes"]}
        params = {**target, "expectedStateVersion": 3, "before": before,
                  ...
                  "gridReference": {"tempoBpm": 120.0,
                                    "timeSignature": {"numerator": 4, "denominator": 4},
                                    "setFingerprint": dispatch_request(song, {"method": "get_live_state"}, 3)["setFingerprint"]},
                  "operation": "apply_midi_diatonic_harmony", "midiDiatonicHarmony": harmony,
                  "removeNoteIds": [], "newNotes": new_notes}
        tampered = copy.deepcopy(params)
        tampered["newNotes"][0]["pitch"] = 65
        tampered["midiDiatonicHarmony"]["newNotes"][0]["pitch"] = 65
        with self.assertRaisesRegex(ValueError, "diatonic-harmony new notes"):
            dispatch_request(song, {"method": "replace_midi_notes", "params": tampered}, 3)
        result = dispatch_request(song, {"method": "replace_midi_notes", "params": params}, 3)
        self.assertEqual(result["removedNoteIds"], [])
        self.assertEqual(result["addedNoteIds"], [100, 101, 102, 103])
        added = {note["noteId"]: note for note in result["notes"]}
        self.assertEqual([added[note_id]["pitch"] for note_id in (100, 101, 102, 103)], [64, 67, 67, 71])
    ...
    def test_functional_progression_creation_rederives_signed_harmony_before_writing(self):
        song = Song()
        target = {"trackId": "track-1", "clipId": "track-1:clip-0"}
        before_context = dispatch_request(song, {"method": "get_song_musical_context"}, 1)
        before_slot = dispatch_request(song, {"method": "list_clips", "params": {
        ...
        progression = {
            "scale": {"name": "Major", "rootNote": 0, "rootName": "C",
                      ...
                      "noteNames": ["C", "D", "E", "F", "G", "A", "B"],
                      "degrees": [1, 2, 3, 4, 5, 6, 7], "family": "major"},
            "scaleSize": 7, "voiceLeading": "root_position", "notesPerChord": 3,
        ...
        params = {**target, "expectedStateVersion": 1,
                  "operation": "create_scale_chord_progression_clip", "name": "A7 Fm G",
        ...
        tampered_song = Song()
        tampered = {**params, "notes": [{**notes[0], "pitch": 58}, *notes[1:]],
                    "musicalContext": dispatch_request(tampered_song, {"method": "get_song_musical_context"}, 1),
                    "before": dispatch_request(tampered_song, {"method": "list_clips", "params": {
                        "trackId": "track-1"}}, 1)["clips"][0]}
        with self.assertRaisesRegex(ValueError, "functional progression"):
            dispatch_request(tampered_song, {"method": "create_midi_clip", "params": tampered}, 1)
        self.assertFalse(tampered_song.tracks[1].clip_slots[0].has_clip)
